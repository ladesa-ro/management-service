...
- [x] **Container `ladesa-rabbitmq` órfão removido**: concluído em 2026-08-22. A mensageria já tinha migrado de RabbitMQ/Rascal pra BullMQ sobre PostgreSQL, mas o container e as variáveis `MESSAGE_BROKER_*` continuavam em `.docker/compose.yml`/`.env`/`.env.example`. Removido o serviço do compose e substituídas as variáveis pelas que o código de fato lê (`QUEUE_DATABASE_URL`, `QUEUE_SCHEMA`, `QUEUE_TIMETABLE_GENERATE`, `QUEUE_FOLHA_PONTO_WHATSAPP`) — sem elas a fila ficava indisponível em dev (`QueueOptionsProvider` resolvia `null`).
