...
import { CalendarioIndisponibilidadeProfessorCreateCommandHandlerImpl } from "./calendario-indisponibilidade-professor-create.command.handler";
...
function createRepository() {
  return { ...createMockCqrsRepository(), findAllAtivasByPerfilId: vi.fn().mockResolvedValue([]) };
}

describe("CalendarioIndisponibilidadeProfessorCreateCommandHandlerImpl", () => {
  function createHandler(overrides: { repository?: object; permissionChecker?: object } = {}) {
    ...
    const handler = new CalendarioIndisponibilidadeProfessorCreateCommandHandlerImpl(
      repository as any,
  ...
  it("should create and return the result from repository.getFindOneQueryResult", async () => {
    ...
    const expectedResult = { id, tipo: CalendarioIndisponibilidadeProfessorTipo.BLOQUEIO };
