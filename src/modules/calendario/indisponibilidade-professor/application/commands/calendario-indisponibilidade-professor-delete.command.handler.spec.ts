...
import { CalendarioIndisponibilidadeProfessorDeleteCommandHandlerImpl } from "./calendario-indisponibilidade-professor-delete.command.handler";

function createRepository() {
  return { ...createMockCqrsRepository(), findAllAtivasByPerfilId: vi.fn().mockResolvedValue([]) };
}

describe("CalendarioIndisponibilidadeProfessorDeleteCommandHandlerImpl", () => {
  function createHandler(overrides: { repository?: object; permissionChecker?: object } = {}) {
    ...
    const handler = new CalendarioIndisponibilidadeProfessorDeleteCommandHandlerImpl(
      repository as any,
