/**
 * Calendario Indisponibilidade Ambiente — definicao dos campos (FieldMetadata) da entidade.
 */
import { z } from "zod";
import { createFieldMetadata, createSchema } from "@/domain/abstractions";

export const CalendarioIndisponibilidadeAmbienteTipoValues = ["BLOQUEIO", "PREFERENCIA"] as const;

export const CalendarioIndisponibilidadeAmbienteTipoSchema = z.enum(
  CalendarioIndisponibilidadeAmbienteTipoValues,
);

export const CalendarioIndisponibilidadeAmbienteFields = {
  ambiente: createFieldMetadata({
  ...
  tipo: createFieldMetadata({
    ...
    schema: createSchema(() => CalendarioIndisponibilidadeAmbienteTipoSchema),
  }),
