...
export class RenameDiaCalendarioToCalendarioLetivoDia1773000000004 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TRIGGER IF EXISTS change_date_updated_table_dia_calendario ON calendario_letivo_dia`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP TRIGGER IF EXISTS change_date_updated_table_calendario_letivo_dia ON dia_calendario`,
    );
