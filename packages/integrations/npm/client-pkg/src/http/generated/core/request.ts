...
export const getHeaders = async <T>(config: OpenAPIConfig, options: ApiRequestOptions<T>): Promise<Headers> => {
  ...
  if (options.body !== undefined) {
    if (options.mediaType) {
      ...
      headers["Content-Type"] = options.body.type || "api/octet-stream";
...
export const getRequestBody = (options: ApiRequestOptions): unknown => {
  if (options.body !== undefined) {
    if (options.mediaType?.includes("api/json") || options.mediaType?.includes("+json")) {
...
export const getResponseBody = async (response: Response): Promise<unknown> => {
  if (response.status !== 204) {
    try {
      ...
      if (contentType) {
        const binaryTypes = ["api/octet-stream", "api/pdf", "api/zip", "audio/", "image/", "video/"];
        if (contentType.includes("api/json") || contentType.includes("+json")) {
