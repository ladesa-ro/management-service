import { GenericListInput, PaginateQueryAdapter } from "@/application/standards/ladesa-spec/search/adapter";
...
export const LadesaSearch = async <T extends ObjectLiteral>(path: string, dto: GenericListInput | null, qb: SelectQueryBuilder<T>, config: PaginateConfig<T>) => {
  const paginateQuery = PaginateQueryAdapter.FromGenericListInput(path, dto);
